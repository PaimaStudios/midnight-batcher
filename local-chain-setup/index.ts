...
if (volumeExists) {
  children.push(spawn('docker', ['compose', 'up'], { stdio: 'inherit' }));
}
else {
  console.log("starting node 0.7.0");

  const process = spawn('docker', ['compose', '-f', 'docker-compose-0.7.0.yml', 'up']);

  children.push(process);

  const killProcess070 = (): Promise<boolean> => new Promise((resolve, reject) => {
    process.on('close', () => {
      console.log("Node 0.7.0 did exit");
      resolve(true);
    });
  ...
  wallet.close();
  await killProcess070();
}


execSync(["docker", "compose", "-f", "docker-compose-0.7.0.yml", "down"].join(" "));

console.log("starting node 0.8.0");
